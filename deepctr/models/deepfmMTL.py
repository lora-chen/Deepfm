...
def DeepFMMTL(linear_feature_columns, dnn_feature_columns, embedding_size=8, use_fm=True, dnn_hidden_units=(128, 128),
    ...
    label_number
    output1 = tf.keras.layers.Dense(label_number[0],activation='softmax',name = "output1")(final_logit)
    output2 = tf.keras.layers.Dense(label_number[1],activation='softmax',name = "output2")(final_logit)

    # output1 = tf.keras.layers.Dense(1, activation='softmax', name='output1')(final_logit)
    # output1 = tf.reshape(output1, (-1, 1))
    # output2 = tf.keras.layers.Dense(1, activation='softmax', name='output2')(final_logit)
    # output2 = tf.reshape(output2, (-1, 1))
    model = tf.keras.models.Model(inputs=inputs_list, outputs=[output1,output2])
    return model